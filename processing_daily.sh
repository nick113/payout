...
if [ "$(ls -A $1)" ]; then
	...
	do
	  ...
	  read PAYOUTS < $f
	  /usr/local/bin/electrum -w .electrum/wallets/wallet_22_06_17 paytomany -W JjEDMht2Nwpt5Kux -f 0.095 "$PAYOUTS"  > result.txn 2>>$3/processing.log ;
	...
	if tail -n 4 $3/processing.log | grep -q true ; then
		...
		tail -n 11 $3/processing.log | grep error  | mail -s 'Error in file '$f  log14@qq.com
